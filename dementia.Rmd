...
dementia_tidy <- dementia_tidy %>% 
  mutate(CDR_2 = ifelse(CDR > 0, 1, CDR ))
```
...
#Accuracy is only 74.25%. Can be 95% certain that this lies between 68.58% and 79.38%. This is significantly better than by predicting the most common class. 
...
confusionMatrix(plda_test$class,
               factor(test$CDR_2))
#Does not run - no 2s in test set as too small!!

#Test on whole dataset
plda_whole <- dementia_no_na %>% 
  select(years_of_education,
         socioeconomic_status,
         age,
         MMSE_score,
